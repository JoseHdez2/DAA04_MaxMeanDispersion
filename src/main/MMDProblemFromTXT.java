...
public abstract class MMDProblemFromTXT {
    ...
    private static MaxMeanDispersionProblem parseTXT(String content){
        ...
        /*
        // Infer size of matrix from number of tokens in first line.
        int size = lines.get(0).split(SEPARATOR).length;
        
        float[][] distMat = new float[size][size];
        
        for (int i = 0; i < lines.size(); i++){
            String[] tokenLine = lines.get(i).split(SEPARATOR);
            for (int j = 0; j < lines.get(i).length(); j++){
                distMat[i][j] = Float.valueOf(tokenLine[j]);
            }
        }*/
        
        return new MaxMeanDispersionProblem(distMat);
    }
